...
export function createApp(config: AppConfig, supplied: AppDependencies = {}) {
  ...
  app.post("/api/people/:personId/relationships", requireMember, async (request, response, next) => {
    ...
    try {
      ...
      return await data!.addFamilyRelationship(request.session.userId!, firstPersonId, secondPersonId, relationshipType, dateText)
        ? response.status(201).json({ ok: true }) : response.status(404).json({ error: "person_not_found" });
    } catch (error) { next(error); }
  });
  ...
  app.get("/admin/activity", requireAdministrator, async (_request, response, next) => {
    try {
      ...
      const labels = { login: "Signed in", photo_viewed: "Viewed photo", photo_tagged: "Tagged photo", photo_untagged: "Removed photo tag", photo_notes_updated: "Updated photo notes" } as const;
      const rows = activity.map((event) => `<tr><td>${new Date(event.occurredAt).toLocaleString("en-US")}</td><td>${escapeHtml(event.userName)}<br><small>${escapeHtml(event.userEmail)}</small></td><td>${labels[event.eventType]}</td><td>${event.photoName ? escapeHtml(event.photoName) : "—"}</td></tr>`).join("");
      response.type("html").send(page(`<p class="eyebrow">Administration</p><h1>Recent activity</h1><p>Successful sign-ins and member interactions with photographs.</p><table><thead><tr><th>When</th><th>Member</th><th>Activity</th><th>Photograph</th></tr></thead><tbody>${rows || '<tr><td colspan="4">No activity recorded yet.</td></tr>'}</tbody></table><p><a href="/admin/members">Manage members</a> · <a href="/app">Back to archive</a></p>`));
    } catch (error) { next(error); }
  });
