...
export function createApp(config: AppConfig, supplied: AppDependencies = {}) {
  ...
  app.get("/auth/google/callback", async (request, response, next) => {
    ...
    try {
      ...
      await data.recordActivity(user.id, "login");
  ...
  app.post("/api/drive/folders/:folderId/remove", requireMember, async (request, response, next) => {
    try {
      const removed = await data!.detachDriveFolder(request.session.userId!, String(request.params.folderId));
      return removed
        ? response.redirect(303, "/drive/folders?removed=1")
        : response.status(404).json({ error: "folder_not_found" });
    } catch (error) { next(error); }
  });
  ...
  app.post("/api/drive/folders/:folderId/photos/:fileId/view", requireMember, async (request, response, next) => {
    try {
      return await data!.recordActivity(request.session.userId!, "photo_viewed", String(request.params.folderId), String(request.params.fileId))
        ? response.status(204).end()
  ...
  app.put("/api/drive/folders/:folderId/photos/:fileId/text", requireMember, async (request, response, next) => {
    ...
    try {
      ...
      if (!record) return response.status(404).json({ error: "photo_not_found" });
      await data!.recordActivity(request.session.userId!, "photo_notes_updated", String(request.params.folderId), String(request.params.fileId), { captionUpdated: true, notesPresent: Boolean(notes) });
  ...
  app.post("/api/people", requireMember, async (request, response, next) => {
    ...
    try {
      const created = await data!.createPersonWithAlias(request.session.userId!, alias);
      await data!.recordActivity(request.session.userId!, "family_person_created", undefined, undefined, { alias, aliasId: created.aliasId }, created.personId);
  ...
  app.post("/api/people/:personId/aliases", requireMember, async (request, response, next) => {
    ...
    try {
      ...
      if (!created) return response.status(404).json({ error: "person_not_found" });
      await data!.recordActivity(request.session.userId!, "family_alias_added", undefined, undefined, { alias, aliasId: created.aliasId }, created.personId);
      return response.status(201).json(created);
    } catch (error) { next(error); }
  });

  app.patch("/api/people/:personId/aliases/:aliasId", requireMember, async (request, response, next) => {
    const alias = typeof request.body.alias === "string" ? request.body.alias.trim() : "";
    if (!alias || alias.length > 200) return response.status(400).json({ error: "valid_alias_required" });
    try {
      const updated = await data!.updatePersonAlias(request.session.userId!, String(request.params.personId), String(request.params.aliasId), alias);
      if (!updated) return response.status(404).json({ error: "alias_not_found" });
      await data!.recordActivity(request.session.userId!, "family_alias_updated", undefined, undefined, { alias, aliasId: updated.aliasId }, updated.personId);
      return response.json(updated);
    } catch (error) {
      if (typeof error === "object" && error !== null && "code" in error && error.code === "23505") return response.status(409).json({ error: "alias_already_exists" });
      next(error);
    }
  });
  ...
  app.post("/api/drive/folders/:folderId/photos/:fileId/subjects", requireMember, async (request, response, next) => {
    ...
    try {
      ...
      if (!region) return response.status(404).json({ error: "photo_not_found" });
      await data!.recordActivity(request.session.userId!, "photo_tagged", String(request.params.folderId), String(request.params.fileId), { regionId: region.id, subjectType: region.subjectType, label: region.label });
  ...
  app.delete("/api/drive/folders/:folderId/photos/:fileId/subjects/:regionId", requireMember, async (request, response, next) => {
    try {
      const folderId = String(request.params.folderId), fileId = String(request.params.fileId), regionId = String(request.params.regionId);
      if (!await data!.deletePhotoSubjectRegion(request.session.userId!, folderId, fileId, regionId)) return response.status(404).json({ error: "subject_region_not_found" });
      await data!.recordActivity(request.session.userId!, "photo_untagged", folderId, fileId, { regionId });
  ...
  app.post("/api/people/:personId/relationships", requireMember, async (request, response, next) => {
    const requestedType = request.body.relationshipType;
    const relationshipType = requestedType === "spouse" ? "spouse" : requestedType === "child" || requestedType === "parent" ? "parent" : null;
    const relatedPersonId = typeof request.body.relatedPersonId === "string" ? request.body.relatedPersonId : "";
    const dateText = typeof request.body.dateText === "string" && request.body.dateText.trim() ? request.body.dateText.trim() : null;
    if (!relationshipType || !relatedPersonId || (dateText?.length ?? 0) > 100) return response.status(400).json({ error: "valid_relationship_required" });
    try {
      const focusedPersonId = String(request.params.personId);
      const firstPersonId = requestedType === "parent" ? relatedPersonId : focusedPersonId;
      const secondPersonId = requestedType === "parent" ? focusedPersonId : relatedPersonId;
      if (!await data!.addFamilyRelationship(request.session.userId!, firstPersonId, secondPersonId, relationshipType, dateText)) return response.status(404).json({ error: "person_not_found" });
      await data!.recordActivity(request.session.userId!, "family_relationship_added", undefined, undefined, { relationshipType: requestedType, relatedPersonId, dateText }, focusedPersonId);
      return response.status(201).json({ ok: true });
    } catch (error) { next(error); }
  });

  app.delete("/api/people/:personId/relationships/:relationshipId", requireMember, async (request, response, next) => {
    try {
      const personId = String(request.params.personId), relationshipId = String(request.params.relationshipId);
      if (!await data!.deleteFamilyRelationship(request.session.userId!, personId, relationshipId)) return response.status(404).json({ error: "relationship_not_found" });
      await data!.recordActivity(request.session.userId!, "family_relationship_removed", undefined, undefined, { relationshipId }, personId);
      return response.status(204).end();
    } catch (error) { next(error); }
  });

  app.get("/api/stories", requireMember, async (request, response, next) => {
    try { response.json({ stories: await data!.listFamilyStories(request.session.userId!) }); }
    catch (error) { next(error); }
  });

  app.post("/api/stories", requireMember, async (request, response, next) => {
    const title = typeof request.body.title === "string" ? request.body.title.trim() : "";
    const body = typeof request.body.body === "string" ? request.body.body.trim() : "";
    const personIds = Array.isArray(request.body.personIds) ? request.body.personIds.filter((id: unknown): id is string => typeof id === "string") : [];
    if (!title || title.length > 500 || !body || body.length > 100000 || !personIds.length || personIds.length > 100) return response.status(400).json({ error: "valid_story_required" });
    try {
      const story = await data!.createFamilyStory(request.session.userId!, title, body, personIds);
      return story ? response.status(201).json(story) : response.status(404).json({ error: "person_not_found" });
    } catch (error) { next(error); }
  });

  app.get("/stories", requireMember, async (request, response, next) => {
    try {
      const stories = await data!.listFamilyStories(request.session.userId!);
      const cards = stories.map((story) => `<article class="story-card"><h2>${escapeHtml(story.title)}</h2><p class="story-byline">${story.createdBy ? `Shared by ${escapeHtml(story.createdBy)} · ` : ""}${new Date(story.createdAt).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</p><div class="story-body">${escapeHtml(story.body).replace(/\r?\n/g, "<br>")}</div><p class="story-people"><strong>People:</strong> ${story.people.map((person) => `<a href="/people?person=${encodeURIComponent(person.id)}">${escapeHtml(person.primaryName)}</a>`).join(", ")}</p></article>`).join("");
      return response.type("html").send(page(`<style>.stories-layout{display:grid;grid-template-columns:minmax(280px,1fr) minmax(0,2fr);gap:2rem}.story-form,.story-card{background:#f7f2e9;padding:1rem;border-radius:.7rem}.story-form{align-self:start;position:sticky;top:1rem}.story-card{margin-bottom:1rem}.story-card h2{margin-top:0}.story-byline{color:#6d655b;font-size:.9rem}.story-body{line-height:1.65}.linked-people{display:flex;gap:.4rem;flex-wrap:wrap;margin:.5rem 0}.linked-person{background:#e7ddcc;padding:.3rem .55rem;border-radius:1rem}.person-results{padding:0;list-style:none}.person-results button{width:100%;text-align:left;margin:.2rem 0}@media(max-width:800px){.stories-layout{grid-template-columns:1fr}.story-form{position:static}}</style><p class="eyebrow">Family archive</p><h1>Stories</h1><p>Preserve memories and accounts that belong to people, rather than to a single photograph.</p><div class="stories-layout"><form class="story-form"><h2>Add a story</h2><label>Title<input name="title" maxlength="500" required placeholder="A name for this story"></label><label>Story<textarea name="body" maxlength="100000" rows="10" required placeholder="Write what the family remembers…"></textarea></label><label>Link people<input class="story-person-query" maxlength="200" placeholder="Search by name or alias"></label><button class="find-story-people secondary" type="button">Find people</button><ul class="person-results"></ul><div class="linked-people"></div><button type="submit">Save story</button> <span class="story-status muted" role="status"></span></form><section class="story-list">${cards || "<p>No stories have been added yet.</p>"}</section></div><script>const form=document.querySelector('.story-form'),query=form.querySelector('.story-person-query'),results=form.querySelector('.person-results'),linked=form.querySelector('.linked-people'),status=form.querySelector('.story-status'),selected=new Map;function drawLinked(){linked.replaceChildren(...[...selected.values()].map(person=>{const chip=document.createElement('button');chip.type='button';chip.className='linked-person';chip.textContent=person.primaryName+' ×';chip.title='Remove '+person.primaryName;chip.onclick=()=>{selected.delete(person.id);drawLinked()};return chip}))}form.querySelector('.find-story-people').onclick=async()=>{const value=query.value.trim();if(!value)return;const response=await fetch('/api/people/search?q='+encodeURIComponent(value)),people=(await response.json()).people;results.replaceChildren(...(people.length?people.map(person=>{const li=document.createElement('li'),button=document.createElement('button');button.type='button';button.className='secondary';button.textContent=person.primaryName+(person.aliases.length>1?' — '+person.aliases.filter(alias=>alias!==person.primaryName).join(', '):'');button.onclick=()=>{selected.set(person.id,person);drawLinked();results.replaceChildren();query.value=''};li.append(button);return li}):[Object.assign(document.createElement('li'),{textContent:'No matching people'})]))};form.onsubmit=async event=>{event.preventDefault();if(!selected.size){status.textContent='Link at least one person.';return}status.textContent='Saving…';const response=await fetch('/api/stories',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({title:form.elements.title.value,body:form.elements.body.value,personIds:[...selected.keys()]})});if(response.ok)location.reload();else status.textContent='The story could not be saved.'};</script>`));
    } catch (error) { next(error); }
  });

  app.get("/people", requireMember, async (_request, response) => response.type("html").send(page(`
    <style>.people-explorer{display:grid;grid-template-columns:minmax(0,3fr) minmax(360px,2fr);gap:1.5rem;min-height:70vh}.people-photos,.person-panel{min-width:0}.people-photo-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:1rem;max-height:68vh;overflow-y:auto}.person-photo{display:grid;gap:.4rem;padding:.55rem;background:#f3eee5;border-radius:.6rem;color:#29251f;text-decoration:none;font-size:.85rem}.person-photo img{width:100%;height:130px;object-fit:cover;border-radius:.35rem}.person-search{display:flex;align-items:end;margin:0 0 1rem}.person-search label{flex:1}.search-results,.family-list{display:grid;gap:.4rem;padding:0;list-style:none}.person-link{width:100%;text-align:left;background:#f3eee5;color:#493a29}.person-panel section{border-top:1px solid #e9e0d4;margin-top:1rem;padding-top:.7rem}.person-aliases{color:#6d655b}.alias-editor{margin:.7rem 0}.alias-editor summary{cursor:pointer;color:#765b38}.alias-list{display:grid;gap:.5rem;margin:.7rem 0}.alias-row{display:flex;align-items:end;gap:.4rem}.alias-row label{flex:1}.alias-row button{white-space:nowrap}.alias-status{display:block;min-height:1.5em}@media(max-width:800px){.people-explorer{grid-template-columns:1fr}.people-photo-grid{max-height:none}}</style>
    <p class="eyebrow">Family explorer</p><div class="people-explorer"><section class="people-photos"><h1 class="photo-heading">Choose a person</h1><p class="photo-summary">Search by any known name or alias.</p><div class="people-photo-grid"></div></section><aside class="person-panel"><form class="person-search"><label>Find a person<input class="person-query" maxlength="200" placeholder="Name or alias"></label><button type="submit">Search</button></form><ul class="search-results"></ul><div class="person-detail" hidden><h2 class="person-name"></h2><p class="person-aliases"></p><details class="alias-editor"><summary>Edit names and aliases</summary><div class="alias-list"></div><span class="alias-status muted" role="status"></span></details><section><strong>Parents</strong><ul class="family-list parents"></ul></section><section><strong>Marriages</strong><ul class="family-list spouses"></ul></section><section><strong>Children</strong><ul class="family-list children"></ul></section></div><p><a href="/app">Back to archive</a></p></aside></div>
    <script>const searchForm=document.querySelector(".person-search"),query=document.querySelector(".person-query"),results=document.querySelector(".search-results"),detail=document.querySelector(".person-detail"),photoGrid=document.querySelector(".people-photo-grid"),heading=document.querySelector(".photo-heading"),summary=document.querySelector(".photo-summary");let focusedPersonId=null;function personButton(person){const li=document.createElement("li"),button=document.createElement("button");button.type="button";button.className="person-link";button.textContent=person.primaryName+(person.aliases.length>1?" — "+person.aliases.filter(a=>a!==person.primaryName).join(", "):"")+(person.relationshipDate?" ("+person.relationshipDate+")":"")+"    View →";button.addEventListener("click",()=>focusPerson(person.id));li.append(button);return li}function family(target,people){target.replaceChildren(...(people.length?people.map(personButton):[Object.assign(document.createElement("li"),{textContent:"None recorded"})]))}async function focusPerson(id){heading.textContent="Loading person…";summary.textContent="";const response=await fetch("/api/people/"+encodeURIComponent(id)+"/explorer");if(!response.ok){heading.textContent="That person could not be opened";summary.textContent="Please try again or choose another search result.";return}const person=await response.json();focusedPersonId=id;history.replaceState(null,"","/people?person="+encodeURIComponent(id));detail.hidden=false;document.querySelector(".person-name").textContent=person.primaryName;document.querySelector(".person-aliases").textContent=person.aliases.length>1?"Also known as: "+person.aliases.filter(a=>a!==person.primaryName).join(", "):"";family(document.querySelector(".parents"),person.parents);family(document.querySelector(".spouses"),person.spouses);family(document.querySelector(".children"),person.children);heading.textContent=person.primaryName;summary.textContent=person.photos.length+" identified "+(person.photos.length===1?"photo":"photos");photoGrid.replaceChildren(...person.photos.map(photo=>{const link=document.createElement("a"),img=document.createElement("img"),caption=document.createElement("span");link.className="person-photo";link.href="/drive/folders/"+encodeURIComponent(photo.folderId)+"/browse?parent="+encodeURIComponent(photo.parentDriveId)+"&photo="+encodeURIComponent(photo.driveFileId);img.loading="lazy";img.src="/api/drive/folders/"+encodeURIComponent(photo.folderId)+"/files/"+encodeURIComponent(photo.driveFileId)+"/thumbnail";caption.textContent=photo.caption;link.append(img,caption);return link}))}function relationshipEditor(listSelector,type,label){const section=document.querySelector(listSelector).parentElement,details=document.createElement("details"),toggle=document.createElement("summary"),formEl=document.createElement("form"),mode=document.createElement("select"),existing=document.createElement("option"),fresh=document.createElement("option"),personInput=document.createElement("input"),dateInput=document.createElement("input"),save=document.createElement("button"),message=document.createElement("span");toggle.textContent=label;existing.value="existing";existing.textContent="Choose an existing person";fresh.value="new";fresh.textContent="Create a new person";mode.append(existing,fresh);personInput.required=true;personInput.maxLength=200;personInput.placeholder="Name or alias";dateInput.maxLength=100;dateInput.placeholder="Marriage date (optional)";save.type="submit";save.textContent=label;message.className="muted";formEl.append(mode,personInput);if(type==="spouse")formEl.append(dateInput);formEl.append(save,message);details.append(toggle,formEl);section.append(details);formEl.addEventListener("submit",async event=>{event.preventDefault();if(!focusedPersonId)return;message.textContent="Saving…";const value=personInput.value.trim();let related;if(mode.value==="new"){const created=await fetch("/api/people",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({alias:value})});if(!created.ok)return message.textContent="The person could not be created.";related=await created.json()}else{const found=await fetch("/api/people/search?q="+encodeURIComponent(value)),people=(await found.json()).people,exact=people.filter(person=>person.aliases.some(alias=>alias.toLowerCase()===value.toLowerCase()));if(exact.length===1)related=exact[0];else if(people.length===1)related=people[0];else return message.textContent=people.length?"More than one person matches. Enter a more specific name.":"No matching person. Choose Create a new person."}const saved=await fetch("/api/people/"+encodeURIComponent(focusedPersonId)+"/relationships",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({relationshipType:type,relatedPersonId:related.id||related.personId,dateText:dateInput.value})});if(!saved.ok)return message.textContent="The relationship could not be saved.";personInput.value="";dateInput.value="";details.open=false;focusPerson(focusedPersonId)})}relationshipEditor(".parents","parent","Add parent");relationshipEditor(".spouses","spouse","Add marriage");relationshipEditor(".children","child","Add child");searchForm.addEventListener("submit",async event=>{event.preventDefault();const value=query.value.trim();if(!value)return;const response=await fetch("/api/people/search?q="+encodeURIComponent(value)),data=await response.json();results.replaceChildren(...(data.people.length?data.people.map(personButton):[Object.assign(document.createElement("li"),{textContent:"No matching person"})]));if(data.people.length===1||data.people.filter(person=>person.aliases.some(alias=>alias.toLowerCase()===value.toLowerCase())).length===1){const exact=data.people.find(person=>person.aliases.some(alias=>alias.toLowerCase()===value.toLowerCase()))||data.people[0];focusPerson(exact.id)}});const initial=new URLSearchParams(location.search).get("person");if(initial)focusPerson(initial);</script>
    <script>const aliasList=document.querySelector(".alias-list"),aliasStatus=document.querySelector(".alias-status"),personName=document.querySelector(".person-name");async function renderAliasEditor(){const personId=new URLSearchParams(location.search).get("person");if(!personId)return;const response=await fetch("/api/people/aliases");if(!response.ok){aliasStatus.textContent="Aliases could not be loaded.";return}const aliases=(await response.json()).aliases.filter(item=>item.personId===personId);aliasList.replaceChildren(...aliases.map(item=>{const form=document.createElement("form"),label=document.createElement("label"),input=document.createElement("input"),save=document.createElement("button");form.className="alias-row";label.textContent=item.isPrimary?"Primary name":"Alias";input.value=item.alias;input.maxLength=200;input.required=true;save.type="submit";save.textContent="Save";label.append(input);form.append(label,save);form.addEventListener("submit",async event=>{event.preventDefault();aliasStatus.textContent="Saving…";save.disabled=true;const updated=await fetch("/api/people/"+encodeURIComponent(personId)+"/aliases/"+encodeURIComponent(item.aliasId),{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({alias:input.value})});save.disabled=false;if(!updated.ok){aliasStatus.textContent=updated.status===409?"That alias is already used for this person.":"The alias could not be saved.";return}aliasStatus.textContent="Alias saved.";await focusPerson(personId)});return form}))}new MutationObserver(renderAliasEditor).observe(personName,{childList:true});</script>
    <script>document.querySelector(".people-photos").id="photos";document.querySelector(".person-detail").id="genealogy";const treeLink=document.createElement("a");treeLink.className="button secondary tree-link";treeLink.textContent="View family tree";treeLink.hidden=true;document.querySelector(".person-name").after(treeLink);new MutationObserver(()=>{const person=new URLSearchParams(location.search).get("person");treeLink.hidden=!person;if(person)treeLink.href="/tree?person="+encodeURIComponent(person)}).observe(document.querySelector(".person-name"),{childList:true});</script>
  `)));

  app.get("/tree", requireMember, async (_request, response) => response.type("html").send(page(`
    <style>
      .tree-shell{max-width:70rem}.tree-root,.tree-node{border-left:3px solid #cbbba5;margin:.7rem 0;padding-left:1rem}.tree-root{border-left-color:#765b38}.tree-node summary{cursor:pointer;font-weight:700;padding:.5rem;border-radius:.45rem;background:#f3eee5}.tree-node[open]>summary{background:#e9e0d4}.tree-actions{display:flex;gap:.8rem;flex-wrap:wrap;margin:.45rem 0 .7rem;font-size:.85rem}.tree-branches{display:grid;gap:.7rem}.tree-branch{margin-left:.6rem}.tree-branch>strong{display:block;color:#765b38;margin:.45rem 0}.tree-empty,.tree-repeat{color:#6d655b;font-size:.9rem}.tree-status{padding:1rem;background:#f3eee5;border-radius:.6rem}.tree-relative{position:relative}.remove-relationship{padding:.3rem .5rem;margin:0 0 .35rem 1rem;background:transparent;color:#765b38}.tree-add{margin:.6rem 0}.tree-add summary{cursor:pointer;color:#765b38}.tree-add form{margin:.6rem 0;padding:.7rem;background:#f3eee5;border-radius:.5rem}.tree-add .muted{margin:0}
    </style>
    <div class="tree-shell"><p class="eyebrow">Family tree</p><h1 class="tree-heading">Family tree</h1><p>Open a person to reveal their parents, marriages, and children. Branches load as you expand them.</p><div class="tree-view"><p class="tree-status">Loading family tree…</p></div><p><a href="/people">Find another person</a> · <a href="/app">Back to archive</a></p></div>
    <script>
      const tree=document.querySelector(".tree-view"),heading=document.querySelector(".tree-heading"),rootId=new URLSearchParams(location.search).get("person");
      function actions(person){const row=document.createElement("div"),individual=document.createElement("a"),photos=document.createElement("a");row.className="tree-actions";individual.href="/people?person="+encodeURIComponent(person.id)+"#genealogy";individual.textContent="Individual genealogy form";photos.href="/people?person="+encodeURIComponent(person.id)+"#photos";photos.textContent="Photos"+(Number.isInteger(person.photoCount)?" ("+person.photoCount+")":"");row.append(individual,photos);return row}
      function addEditor(focusedId,type,label){const details=document.createElement("details"),summary=document.createElement("summary"),form=document.createElement("form"),mode=document.createElement("select"),existing=document.createElement("option"),fresh=document.createElement("option"),name=document.createElement("input"),date=document.createElement("input"),save=document.createElement("button"),status=document.createElement("span");details.className="tree-add";summary.textContent="Add "+label.toLowerCase();existing.value="existing";existing.textContent="Choose an existing person";fresh.value="new";fresh.textContent="Create a new person";mode.append(existing,fresh);name.required=true;name.maxLength=200;name.placeholder="Name or alias";date.maxLength=100;date.placeholder="Marriage date (optional)";save.type="submit";save.textContent="Add "+label.toLowerCase();status.className="muted";form.append(mode,name);if(type==="spouse")form.append(date);form.append(save,status);details.append(summary,form);form.addEventListener("submit",async event=>{event.preventDefault();status.textContent="Saving…";const value=name.value.trim();let related;if(mode.value==="new"){const created=await fetch("/api/people",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({alias:value})});if(!created.ok){status.textContent="The person could not be created.";return}related=await created.json()}else{const found=await fetch("/api/people/search?q="+encodeURIComponent(value)),people=(await found.json()).people,exact=people.filter(person=>person.aliases.some(alias=>alias.toLowerCase()===value.toLowerCase()));if(exact.length===1)related=exact[0];else if(people.length===1)related=people[0];else{status.textContent=people.length?"More than one person matches. Enter a more specific name.":"No matching person. Choose Create a new person.";return}}const saved=await fetch("/api/people/"+encodeURIComponent(focusedId)+"/relationships",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({relationshipType:type,relatedPersonId:related.id||related.personId,dateText:date.value})});if(!saved.ok){status.textContent="The relationship could not be saved.";return}location.reload()});return details}
      function branch(label,people,ancestry,focusedId,type){const singular=type==="child"?"Child":type==="spouse"?"Marriage":"Parent",section=document.createElement("section"),title=document.createElement("strong");section.className="tree-branch";title.textContent=label;section.append(title);if(!people.length){const empty=document.createElement("span");empty.className="tree-empty";empty.textContent="None recorded";section.append(empty)}else people.forEach(person=>{const wrapper=document.createElement("div");wrapper.className="tree-relative";wrapper.append(node(person,ancestry));if(person.relationshipId){const remove=document.createElement("button");remove.type="button";remove.className="remove-relationship";remove.textContent="Remove "+singular.toLowerCase()+" relationship";remove.addEventListener("click",async()=>{if(!confirm("Remove this relationship? The person and their photos will remain."))return;const response=await fetch("/api/people/"+encodeURIComponent(focusedId)+"/relationships/"+encodeURIComponent(person.relationshipId),{method:"DELETE"});if(response.ok)location.reload()});wrapper.append(remove)}section.append(wrapper)});section.append(addEditor(focusedId,type,singular));return section}
      function node(person,ancestry=new Set){const repeated=ancestry.has(person.id),details=document.createElement("details"),summary=document.createElement("summary"),body=document.createElement("div");details.className="tree-node";summary.textContent=person.primaryName+(person.relationshipDate?" — "+person.relationshipDate:"");details.append(summary,actions(person),body);if(repeated){const note=document.createElement("p");note.className="tree-repeat";note.textContent="Already shown in this branch.";body.append(note);return details}let loaded=false;details.addEventListener("toggle",async()=>{if(!details.open||loaded)return;loaded=true;body.textContent="Loading…";const result=await fetch("/api/people/"+encodeURIComponent(person.id)+"/explorer");if(!result.ok){body.textContent="This branch could not be loaded.";return}const full=await result.json(),next=new Set(ancestry);next.add(person.id);details.replaceChild(actions({...full,photoCount:full.photos.length}),details.children[1]);body.replaceChildren(branch("Parents",full.parents,next,full.id,"parent"),branch("Marriages",full.spouses,next,full.id,"spouse"),branch("Children",full.children,next,full.id,"child"))});return details}
      async function start(){if(!rootId){tree.innerHTML='<p class="tree-status">Choose a person from the individual genealogy form to open their tree.</p>';return}const result=await fetch("/api/people/"+encodeURIComponent(rootId)+"/explorer");if(!result.ok){tree.innerHTML='<p class="tree-status">That person could not be opened.</p>';return}const person=await result.json();heading.textContent=person.primaryName+" family tree";const root=node({...person,photoCount:person.photos.length});root.classList.add("tree-root");tree.replaceChildren(root);root.open=true}start();
    </script>
  `)));
  ...
  app.get("/api/admin/activity", requireAdministrator, async (request, response, next) => {
    const requestedLimit = Number(request.query.limit ?? 100);
    const limit = Number.isInteger(requestedLimit) ? Math.max(1, Math.min(requestedLimit, 500)) : 100;
    try { response.json({ activity: await data!.listRecentActivity(limit) }); }
    catch (error) { next(error); }
  });
  ...
  app.get("/admin/activity", requireAdministrator, async (_request, response, next) => {
    try {
      ...
      const labels = { login: "Signed in", photo_viewed: "Viewed photo", photo_tagged: "Tagged photo", photo_untagged: "Removed photo tag", photo_notes_updated: "Updated photo notes", family_person_created: "Created person", family_alias_added: "Added person alias", family_alias_updated: "Updated person alias", family_relationship_added: "Added family relationship", family_relationship_removed: "Removed family relationship" } as const;
      const rows = activity.map((event) => `<tr><td>${new Date(event.occurredAt).toLocaleString("en-US")}</td><td>${escapeHtml(event.userName)}<br><small>${escapeHtml(event.userEmail)}</small></td><td>${labels[event.eventType]}</td><td>${event.photoName ? escapeHtml(event.photoName) : event.personName ? escapeHtml(event.personName) : "—"}</td></tr>`).join("");
      response.type("html").send(page(`<p class="eyebrow">Administration</p><h1>Recent activity</h1><p>Successful sign-ins and member changes to photographs and the family tree.</p><table><thead><tr><th>When</th><th>Member</th><th>Activity</th><th>Photograph or person</th></tr></thead><tbody>${rows || '<tr><td colspan="4">No activity recorded yet.</td></tr>'}</tbody></table><p><a href="/admin/members">Manage members</a> · <a href="/app">Back to archive</a></p>`));
    } catch (error) { next(error); }
  });
  ...
  app.get("/app", requireMember, async (request, response, next) => {
    try {
      ...
      const folders = driveConnected ? await data!.listAttachedFolders(request.session.userId!) : [];
      const savedFolders = folders.length ? `<section><h2>Your photo folders</h2><ul>${folders.map((folder) => `<li><a href="/drive/folders/${folder.id}/browse"><strong>${escapeHtml(folder.name)}</strong></a></li>`).join("")}</ul></section>` : "";
      return response.type("html").send(page(`<p class="eyebrow">Private family archive</p><h1>Your photographs</h1><p>You are signed in. ${folders.length ? "Your saved photo folders are ready below." : "Choose the Google Drive folders you want in your archive."}</p>${savedFolders}<p>${driveConnected ? `<a class="button" href="/drive/folders">${folders.length ? "Add or remove folders" : "Choose photo folders"}</a>` : '<a class="button" href="/drive/connect">Connect Google Drive</a>'} <a class="button secondary" href="/people">Explore people and family</a> <a class="button secondary" href="/tutorial">Take the tutorial</a></p>${isAdmin ? '<p><a href="/admin/members">Manage application members</a> · <a href="/admin/activity">View recent activity</a></p>' : ""}<form method="post" action="/auth/logout"><button type="submit" class="secondary">Sign out</button></form>`));
    } catch (error) { next(error); }
  });

  app.get("/tutorial", (_request, response) => {
    return response.type("html").send(page(`
      <style>.tutorial{max-width:68rem}.tutorial-intro{max-width:65ch}.tutorial-progress{height:.45rem;background:#e9e0d4;border-radius:1rem;overflow:hidden}.tutorial-progress span{display:block;width:16.66%;height:100%;background:#765b38;transition:width .25s}.tutorial-count{color:#6d655b;font-size:.85rem}.tutorial-step{display:grid;grid-template-columns:minmax(0,1fr) minmax(18rem,.8fr);gap:clamp(1.5rem,5vw,4rem);align-items:center;min-height:23rem}.tutorial-step[hidden]{display:none}.tutorial-step h2{font:700 clamp(1.8rem,4vw,3rem)/1.1 Georgia,serif;margin:.25rem 0 1rem}.practice-card{box-sizing:border-box;padding:1.5rem;border:1px solid #dfd3c3;border-radius:1rem;background:#faf7f2;box-shadow:0 12px 30px #352d2012}.practice-card h3{font:700 1.35rem Georgia,serif}.practice-card label{margin:.75rem 0}.practice-card input[type=checkbox]{width:auto;margin-right:.5rem}.success-card{text-align:center}.status-icon{display:inline-grid;place-items:center;width:3rem;height:3rem;border-radius:50%;background:#dcebd9;color:#2c6425;font-size:1.5rem}.practice-folders{padding-left:0;list-style:none}.practice-image{display:grid;place-items:center;aspect-ratio:4/3;margin-bottom:1rem;border-radius:.5rem;background:linear-gradient(145deg,#b8a68e,#6c5740);color:white;font:700 1.2rem Georgia,serif}.tutorial-actions{display:flex;justify-content:space-between;margin-top:1.5rem}.tutorial-exit{text-align:center}.tutorial button:disabled{opacity:.45;cursor:not-allowed}@media(max-width:900px){.tutorial-step{grid-template-columns:1fr;min-height:0}.practice-card{margin-top:.5rem}}</style>
      <div class="tutorial" data-step="0">
        <p class="eyebrow">Interactive introduction</p>
        <h1>Learn the family archive</h1>
        <p class="tutorial-intro">This practice tour uses examples only. You can complete it without signing in or connecting Google Drive.</p>
        <div class="tutorial-progress" aria-label="Tutorial progress"><span></span></div>
        <p class="tutorial-count" aria-live="polite">Step 1 of 6</p>

        <section class="tutorial-step" data-title="Sign in with Google">
          <div><p class="eyebrow">Step 1</p><h2>Identify yourself with Google</h2><p>On the welcome screen, choose <strong>Continue with Google</strong>. Google verifies your name and email so the archive knows which family member is visiting.</p><p>Signing in does not give the archive access to your photographs. Google Drive is connected separately in a later step.</p></div>
          <div class="practice-card"><p class="eyebrow">Practice screen</p><h3>Welcome to the archive</h3><button type="button" class="practice-google">G&nbsp;&nbsp; Continue with Google</button><p class="muted">For this tutorial, the button is only a demonstration.</p></div>
        </section>

        <section class="tutorial-step" data-title="Return to the archive" hidden>
          <div><p class="eyebrow">Step 2</p><h2>Google returns you safely</h2><p>After you choose your Google account, you return to the archive. Only family members whose email address has been approved can enter.</p><p>Your secure session is remembered, so you usually won’t need to sign in again on the same device.</p></div>
          <div class="practice-card success-card"><span class="status-icon">✓</span><h3>You are signed in</h3><p>Your family archive is ready.</p></div>
        </section>

        <section class="tutorial-step" data-title="Connect Google Drive" hidden>
          <div><p class="eyebrow">Step 3</p><h2>Connect photographs separately</h2><p>Choose <strong>Connect Google Drive</strong> when you’re ready to use photographs stored there. This is separate from signing in.</p><p>The archive asks for read-only access: it can display your selected photographs, but it cannot change or delete files in Google Drive.</p></div>
          <div class="practice-card"><h3>Your photographs</h3><button type="button">Connect Google Drive</button><p class="muted">You remain in control of which folders are selected.</p></div>
        </section>

        <section class="tutorial-step" data-title="Choose folders" hidden>
          <div><p class="eyebrow">Step 4</p><h2>Choose photo folders</h2><p>Select one or more folders in the Google picker. The archive remembers these selections for your account and shows them whenever you return.</p><p>You can add more folders later. Removing a folder from the archive never deletes it from Google Drive.</p></div>
          <div class="practice-card folder-practice"><h3>Choose a folder</h3><label><input type="checkbox" checked> Family photographs</label><label><input type="checkbox"> Grandma’s albums</label><label><input type="checkbox"> Holiday scans</label><p class="folder-feedback muted">1 folder selected</p></div>
        </section>

        <section class="tutorial-step" data-title="Browse photographs" hidden>
          <div><p class="eyebrow">Step 5</p><h2>Open a saved folder</h2><p>Your selected folders appear on the archive screen as soon as you sign in. Choose a folder name to browse its subfolders and photographs.</p><p>Use <strong>Add or remove folders</strong> whenever you want to change the collection.</p></div>
          <div class="practice-card"><h3>Your photo folders</h3><ul class="practice-folders"><li>📁 <a href="#practice-photo">Family photographs</a></li><li>📁 <a href="#practice-photo">Grandma’s albums</a></li></ul><button type="button" class="secondary">Add or remove folders</button></div>
        </section>

        <section class="tutorial-step" data-title="Preserve the story" hidden>
          <div><p class="eyebrow">Step 6</p><h2>Keep the story with the photograph</h2><p>Open a photograph to add a caption and family story. Use <strong>Mark a subject</strong> to draw around a person or meaningful object and give it a name.</p><p>People you identify become searchable in <strong>People &amp; family</strong>, where family relationships and related photographs come together.</p></div>
          <div class="practice-card photo-practice" id="practice-photo"><div class="practice-image" aria-label="Example family photograph">Family photograph</div><label>Caption<input value="Claire and Larry at the lake"></label><label>Notes<textarea rows="3">Summer picnic, 1952.</textarea></label><button type="button">Save text</button></div>
        </section>

        <div class="tutorial-actions"><button type="button" class="secondary tutorial-back" disabled>Back</button><button type="button" class="tutorial-next">Next</button></div>
        <p class="tutorial-exit"><a href="/">Exit tutorial</a></p>
      </div>
      <script>
        (()=>{const root=document.querySelector('.tutorial'),steps=[...root.querySelectorAll('.tutorial-step')],back=root.querySelector('.tutorial-back'),next=root.querySelector('.tutorial-next'),count=root.querySelector('.tutorial-count'),bar=root.querySelector('.tutorial-progress span');let current=0;function show(index){current=Math.max(0,Math.min(steps.length-1,index));steps.forEach((step,i)=>step.hidden=i!==current);root.dataset.step=String(current);count.textContent='Step '+(current+1)+' of '+steps.length+': '+steps[current].dataset.title;bar.style.width=((current+1)/steps.length*100)+'%';back.disabled=current===0;next.textContent=current===steps.length-1?'Finish tutorial':'Next';steps[current].querySelector('button,input,a,textarea')?.focus()}back.addEventListener('click',()=>show(current-1));next.addEventListener('click',()=>current===steps.length-1?location.assign('/'):show(current+1));document.addEventListener('keydown',event=>{if(event.key==='ArrowRight')show(current+1);if(event.key==='ArrowLeft')show(current-1)});root.querySelector('.practice-google').addEventListener('click',()=>show(1));root.querySelectorAll('.folder-practice input').forEach(input=>input.addEventListener('change',()=>{const total=root.querySelectorAll('.folder-practice input:checked').length;root.querySelector('.folder-feedback').textContent=total+' '+(total===1?'folder':'folders')+' selected'}));show(0)})();
      </script>
    `));
  });
...
function page(content: string) {
  content = `<nav aria-label="Main navigation" style="display:flex;gap:1rem;flex-wrap:wrap;margin-bottom:1.5rem;padding-bottom:1rem;border-bottom:1px solid #e9e0d4"><a href="/app">Archive</a><a href="/drive/folders">Photo folders</a><a href="/people">People &amp; family</a><a href="/stories">Stories</a><a href="/tutorial">Tutorial</a></nav>${content}`;
...
function renderRegions(){subjectList.replaceChildren();regions.forEach((region,index)=>{const li=document.createElement("li"),text=document.createElement(region.subjectType==="person"?"a":"span"),remove=document.createElement("button");text.textContent=(index+1)+". "+(region.subjectType==="person"?"Person: ":"Thing: ")+region.label;if(region.subjectType==="person"&&region.personId)text.href="/people?person="+encodeURIComponent(region.personId);remove.type="button";remove.textContent="Remove";remove.addEventListener("click",async()=>{if(!confirm("Remove this marked region?"))return;const response=await fetch(subjectsEndpoint(cards[current])+"/"+encodeURIComponent(region.id),{method:"DELETE"});if(response.ok){regions=regions.filter(item=>item.id!==region.id);renderRegions();drawRegions()}});li.append(text,remove);subjectList.append(li)});if(!drawing)markButton.textContent=regions.length?"Mark another subject":"Mark a subject";subjectHelp.textContent=regions.length?regions.length+" marked "+(regions.length===1?"subject":"subjects")+". Draw another box or review the list below.":"Draw a box around a face, person, or thing.";drawRegions()}
