    ...
    try {
      ...
      const cards = browserPage.items.map((item) => item.mimeType === "application/vnd.google-apps.folder"
        ...
        : item.mimeType.startsWith("image/")
          ? `<button class="photo-card image-card" data-id="${escapeHtml(item.driveFileId)}" data-name="${escapeHtml(item.name)}" data-full="/api/drive/folders/${folderId}/files/${encodeURIComponent(item.driveFileId)}/content"><img loading="lazy" src="/api/drive/folders/${folderId}/files/${encodeURIComponent(item.driveFileId)}/thumbnail" alt=""><span class="thumbnail-caption">${escapeHtml(item.caption ?? item.name)}</span></button>`
