...
export function createApp(config: AppConfig, supplied: AppDependencies = {}) {
  ...
  app.post("/api/people/:personId/relationships", requireMember, async (request, response, next) => {
    ...
    const relationshipType = requestedType === "spouse" ? "spouse" : requestedType === "child" ? "parent" : null;
    ...
    try {
      return await data!.addFamilyRelationship(request.session.userId!, String(request.params.personId), relatedPersonId, relationshipType, dateText)
    ...
    return response.type("html").send(page(`
      ...
      <div class="tutorial" data-step="0">
        ...
        <section class="tutorial-step" data-title="Preserve the story" hidden>
          ...
          <div class="practice-card photo-practice" id="practice-photo"><div class="practice-image" aria-label="Example family photograph">Family photograph</div><label>Caption<input value="Claire and Larry at the lake"></label><label>Story or notes<textarea rows="3">Summer picnic, 1952.</textarea></label><button type="button">Save text</button></div>
        </section>
...
function page(content: string) {
  content = `<nav aria-label="Main navigation" style="display:flex;gap:1rem;flex-wrap:wrap;margin-bottom:1.5rem;padding-bottom:1rem;border-bottom:1px solid #e9e0d4"><a href="/app">Archive</a><a href="/drive/folders">Photo folders</a><a href="/people">People &amp; family</a><a href="/tutorial">Tutorial</a></nav>${content}`;
