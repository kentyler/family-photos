...
function renderRegions(){subjectList.replaceChildren();regions.forEach(region=>{const li=document.createElement("li"),text=document.createElement("span"),remove=document.createElement("button");text.textContent=(region.subjectType==="person"?"Person: ":"Thing: ")+region.label;remove.type="button";remove.textContent="Remove";remove.addEventListener("click",async()=>{if(!confirm("Remove this marked region?"))return;const response=await fetch(subjectsEndpoint(cards[current])+"/"+encodeURIComponent(region.id),{method:"DELETE"});if(response.ok){regions=regions.filter(item=>item.id!==region.id);renderRegions();drawRegions()}});li.append(text,remove);subjectList.append(li)});drawRegions()}
...
function cancelDrawing(){drawing=false;start=null;pendingRect=null;canvas.classList.remove("drawing");editor.hidden=true;markButton.textContent="Mark a subject";subjectStatus.textContent="";drawRegions()}
